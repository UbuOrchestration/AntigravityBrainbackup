...
async function run() {
    ...
    for (const [sku, item] of Object.entries(descriptions)) {
        ...
        try {
            ...
            const ebayItemId = await addFixedPriceItem(sku, item.title, desc, itemSpecificsJson, item.price, 1, [], config);
            
            // Insert into DB as ACTIVE
            await db.run(`
                INSERT INTO inventory 
                (sku, title, listing_description, ebay_item_id, p_ebay, p_source, p_sold, status, quantity, delivery_days)
                VALUES (?, ?, ?, ?, ?, ?, ?, 'ACTIVE', 1, 3)
            `, [sku, item.title, desc, ebayItemId, item.price, item.cost, item.price]);
