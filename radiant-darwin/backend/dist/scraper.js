...
exports.scrapeSourceProduct = scrapeSourceProduct;
 ...
 */
async function scrapeSourceProduct(url, sku) {
 ...
 */
function getSimulatedProduct(url, sku) {
    ...
    if (url.includes('electronics'))
        ...
        name = 'Chefman Digital Air Fryer XL';
    else if (url.includes('toy'))
        name = 'LEGO Star Wars Millennium Falcon Model';
    else if (url.includes('rv') || url.includes('convenience'))
        name = 'Camco Heavy Duty RV Leveling Blocks';
    else if (sku)
        name = `Product Ref: ${sku.toUpperCase()}`;
    return {
        price,
        inStock,
        title: name,
